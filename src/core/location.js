...
export function isResidenceLabel(label) {
  return /^(?:current location|your current location|where are you (?:currently )?(?:located|based)|location of residence|location\s*\(\s*city\s*\)|where do you live(?:\?\s*\(city and state\/province\))?)$/.test(key(label).replace(/[✱*:?]+$/g, '').trim());
}
